...
//spoc
char* spocStatus[]={"PROC_UNINIT","PROC_SLEEPING","PROC_RUNNABLE","PROC_ZOMBIE"};
int spocNum = 0;
...
proc_run(struct proc_struct *proc) {
      cprintf("spoc... run id:%d state:%s\n",proc->pid, spocStatus[proc->state]);
      if (proc != current) {
            bool intr_flag;
            struct proc_struct *prev = current, *next = proc;
            struct proc_struct *spoc = current;
            //cprintf("spoc... before switch:prev: %d state %d next:%d state %d\n",prev->pid,prev->state ,next->pid,next->state);
            local_intr_save(intr_flag);
            {
                  
                  /*
                   if(current->pid == 2 && spocNum == 0)
                   {
                   spocNum = 1;
                   int pid4= kernel_thread(init_main, "spoc... init main4: Hello world!!", 0);
                  ...
                  char temp2[10];
                  strcpy(temp2,spocStatus[next->state]);
                  if(temp1 == 2)
                        strcpy(temp2,"READY");
                  
                  cprintf("spoc1... before switch:prev: %d state %s\n",prev->pid,spocStatus[prev->state]);
                  cprintf("spoc1... before switch:next: %d state %s (actrual: %s)\n",next->pid,spocStatus[next->state],temp2);
                  ...
                  if(temp1 == 2)
                        strcpy(temp2,"READY");
                  cprintf("spoc2... after switch:prev: %d state %s\n",spoc->pid,spocStatus[spoc->state]);
                  cprintf("spoc2... after switch:next: %d state %s (actrual: %s)\n",next->pid,spocStatus[next->state],temp2);
            }
...
do_wait(int pid, int *code_store) {
      ...
      //cprintf("spoc...before do_wait: begin %d state %d\n",proc->pid, proc->state);
repeat:
      ...
      while ((le = list_next(le)) != list) {
            ...
            if (proc != NULL) {
                  ...
                  if (proc->state == PROC_ZOMBIE) {
                        cprintf("spoc...child zombie: id %d state PROC_ZOMBIE\n",proc->pid);
                        goto found;
                  }
            }
      }
      if (haskid) {
            ...
            cprintf("spoc...before sleep: id %d state PROC_SLEEPING\n",proc->pid);
            current->wait_state = WT_CHILD;
            schedule();
            goto repeat;
      }
...
found:
      ...
      local_intr_save(intr_flag);
      {
            cprintf("spoc5...remove_links indicate thread dead, id: %d, state: Dead\n",proc->pid);
            remove_links(proc);
      }
      local_intr_restore(intr_flag);
      put_kstack(proc);
...
do_exit(int error_code) {
      ...
      cprintf("spoc... before do_exit: proc pid %d will exit, state is %s\n", current->pid, spocStatus[current->state]);
      ...
      local_intr_save(intr_flag);
      {
            ...
            if (proc->wait_state == WT_CHILD) {
                  cprintf("spoc... change to parent: pid %d state %s\n", proc->pid, spocStatus[proc->state]);
                  wakeup_proc(proc);
            }
      }
      local_intr_restore(intr_flag);
